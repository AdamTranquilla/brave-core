...
std::unique_ptr<message_center::Notification> CreateLegacyAdNotification(
  ...
  if (base::IsStringUTF8(ad_notification.title)) {
    base::UTF8ToUTF16(ad_notification.title.c_str(),
                      ad_notification.title.length(), &title);
  }

  base::string16 body;
  if (base::IsStringUTF8(ad_notification.body)) {
    base::UTF8ToUTF16(ad_notification.body.c_str(),
                      ad_notification.body.length(), &body);
  }
