...
// static
void AdNotificationPopup::FadeOutAndCloseWidgetView(
  ...
  AdNotificationView* view = popup->GetAdNotificationView();
  DCHECK(view);

  // Destroy the widget when done. The observer deletes itself on completion
  view->FadeOut(base::BindOnce(
      [](const std::string& notification_id) {
        DCHECK(!notification_id.empty());
...
void AdNotificationPopup::Close(const std::string& notification_id,
  ...
  FadeOutAndCloseWidgetView(notification_id);
}
...
void AdNotificationPopup::CreatePopup() {
  ...
  AddChildView(ad_notification_view_);
...
gfx::Point AdNotificationPopup::GetOriginForSize(const gfx::Size& size) {
  if (!profile_->GetPrefs()->HasPrefPath(prefs::kAdNotificationOriginX) ||
      !profile_->GetPrefs()->HasPrefPath(prefs::kAdNotificationOriginY)) {
    return GetDefaultOriginForSize(size);
  }

  const int x = profile_->GetPrefs()->GetInteger(prefs::kAdNotificationOriginX);
  const int y = profile_->GetPrefs()->GetInteger(prefs::kAdNotificationOriginY);
  return gfx::Point(x, y);
}

void AdNotificationPopup::SaveOrigin(const gfx::Point& origin) const {
  profile_->GetPrefs()->SetInteger(prefs::kAdNotificationOriginX, origin.x());
  profile_->GetPrefs()->SetInteger(prefs::kAdNotificationOriginY, origin.y());
}
...
AdNotificationView* AdNotificationPopup::GetAdNotificationView() const {
  return ad_notification_view_;
}
